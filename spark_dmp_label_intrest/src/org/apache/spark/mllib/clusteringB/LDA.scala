...
class LDA private (
  ...
  def run(documents: RDD[(Long, Vector)]): LDAModel = {
    ...
    state.getLDAModel(iterationTimes)  //deleteAllCheckpoints
  }
...
private[clusteringB] object LDA {
   ...
   */
  private[clusteringB] type TopicCounts = BDV[Double]

  private[clusteringB] type TokenCount = Double

  /** Term vertex IDs are {-1, -2, ..., -vocabSize} */
  private[clusteringB] def term2index(term: Int): Long = -(1 + term.toLong)

  private[clusteringB] def index2term(termIndex: Long): Int = -(1 + termIndex).toInt

  private[clusteringB] def isDocumentVertex(v: (VertexId, _)): Boolean = v._1 >= 0

  private[clusteringB] def isTermVertex(v: (VertexId, _)): Boolean = v._1 < 0

  /**
   * Compute gamma_{wjk}, a distribution over topics k.
   */
  private[clusteringB] def computePTopic(
