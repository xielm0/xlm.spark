...
sealed trait LDAOptimizer {
   ...
   */
  private[clusteringB] def initialize(docs: RDD[(Long, Vector)], lda: LDA): LDAOptimizer

  private[clusteringB] def next(): LDAOptimizer

  private[clusteringB] def getLDAModel(iterationTimes: Array[Double]): LDAModel
}
...
final class EMLDAOptimizer extends LDAOptimizer {
   ...
   */
  private[clusteringB] var graph: Graph[TopicCounts, TokenCount] = null
  private[clusteringB] var k: Int = 0
  private[clusteringB] var vocabSize: Int = 0
  private[clusteringB] var docConcentration: Double = 0
  private[clusteringB] var topicConcentration: Double = 0
  private[clusteringB] var checkpointInterval: Int = 10
  private var graphCheckpointer: PeriodicGraphCheckpointer[TopicCounts, TokenCount] = null

  /**
   * Compute bipartite term/doc graph.
   */
  override private[clusteringB] def initialize(docs: RDD[(Long, Vector)], lda: LDA): LDAOptimizer = {
  ...
  override private[clusteringB] def next(): EMLDAOptimizer = {
   ...
   */
  private[clusteringB] var globalTopicTotals: TopicCounts = null
  ...
  override private[clusteringB] def getLDAModel(iterationTimes: Array[Double]): LDAModel = {
    require(graph != null, "graph is null, EMLDAOptimizer not initialized.")
...
final class OnlineLDAOptimizer extends LDAOptimizer {
  ...
  private[clusteringB] def getLambda: BDM[Double] = lambda
   ...
   */
  private[clusteringB] def setLambda(lambda: BDM[Double]): this.type = {
    this.lambda = lambda
    this
  }
   ...
   */
  private[clusteringB] def setGammaShape(shape: Double): this.type = {
    this.gammaShape = shape
    this
  }

  override private[clusteringB] def initialize(
  ...
  override private[clusteringB] def next(): OnlineLDAOptimizer = {
   ...
   * subset.
   */
  private[clusteringB] def submitMiniBatch(batch: RDD[(Long, Vector)]): OnlineLDAOptimizer = {
    iteration += 1
    val k = this.k
   ...
   */
  private[clusteringB] def update(stat: BDM[Double], iter: Int, batchSize: Int): Unit = {
...
private[clusteringB] object OnlineLDAOptimizer {
  /**
   ...
   */
  private[clusteringB] def variationalTopicInference(
                                                     termCounts: Vector,
